...
class Api {
  ...
  protected post<T, P = unknown, R = AxiosResponse<T>>(url: string, params?: P, config?: AxiosRequestConfig): Promise<R> {
    return this.api.post(url, params, config);
  }

  protected patch<T, D = unknown, R = AxiosResponse<T>>(url: string, data?: D, config?: AxiosRequestConfig): Promise<R> {
    return this.api.patch<T, R>(url, data, config);
  }
  protected put<T, D = unknown, R = AxiosResponse<T>>(url: string, data?: D, config?: AxiosRequestConfig): Promise<R> {
